...
%   RhoRsdlRatio      Primal/dual residual ratio in rho update test 残差比率
...
while k <= opt.MaxMainIter && (r > epri | s > edua),
  ...
  if opt.AutoRho,%判断是否进行rho的自适应，默认进行自适应
    %判断迭代的周期是否为0，如果为0相当于不进行自适应。
    if k ~= 1 && mod(k, opt.AutoRhoPeriod) == 0,
        %判断是否进行参数tao的自适应
      if opt.AutoRhoScaling,
        %tao的值，rhorsdltarget为1，残差比的目标
        rhomlt = sqrt(r/(s*opt.RhoRsdlTarget));
        %判断计算出来的tao的范围不能超过RhoScaling，100。
        if rhomlt < 1, rhomlt = 1/rhomlt; end
        if rhomlt > opt.RhoScaling, rhomlt = opt.RhoScaling; end
      else
        %如果不进行tao的参数自适应那么就设置tao的大小为rhoscaling的大小，为100
        %但是tao为100的时候是不收敛的，rho的波动太大。
      ...
      rho = rsf*rho;
       %还原U，U是对偶变量的1/rho。
      U = U/rsf;
      %判断rho是否进行了变化，变化了那么DTD+rho I也变了。
      if rsf ~= 1,
        [luL, luU] = factorise(D, rho);
      end
    end
