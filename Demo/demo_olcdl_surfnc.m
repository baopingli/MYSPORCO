% Script demonstrating usage of the olcdl_surfnc_freq and olcdl_surfnc_sprs
% functions.
%
...
S0(:,:,1) = single(rgbtogrey(imresize(stdimage('monarch'),[512,512])))/255;
%S0(:,:,2) = single(stdimage('barbara.grey')) / 255;
S0(:,:,2) = single(rgbtogrey(imresize(stdimage('barbara'),[512,512])))/255;
%S0(:,:,3) = single(stdimage('kiel.grey')) / 255;
S0(:,:,3) = single(rgbtogrey(imresize(stdimage('boats'),[512,512])))/255;
%S0(:,:,4) = single(rgb2gray(stdimage('mandrill'))) / 255;
S0(:,:,4) = single(rgbtogrey(imresize(stdimage('goldhill'),[512,512])))/255;
%tmp = single(stdimage('man.grey')) / 255;
S0(:,:,5)= single(imresize(stdimage('kiel.grey'),[512,512]))/255;
...
[D, optinf] = olcdl_surfnc_freq(D0, Sh, lambda, opt);
% To use sparse matrix solution method, comment line above and
% uncomment line below
% [D, optinf] = olcdl_surfnc_sprs(D0, Sh, lambda, opt);
